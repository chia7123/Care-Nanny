...
class DeluxeService extends StatelessWidget {
  DeluxeService({Key key}) : super(key: key);
                          ...
                          onDismissed: (direction) {
                            if (direction == DismissDirection.endToStart) {
                              Map<String, dynamic> serviceInfo = {
                                'title': data['deluxe'][index]['title'],
                                'price': data['deluxe'][index]['price'],
                              };
                              _deleteService(serviceInfo);
                            }
                          },
                          key: Key(data['deluxe'][index]['title']),
                          child: ListTile(
                            title: Text(
                                '${index + 1}. ${data['deluxe'][index]['title']}'),
                            subtitle: Text(
                                'RM ${double.parse(data['deluxe'][index]['price']).toStringAsFixed(2)}'),
                          ),
                        ),
                        Divider(
  ...
  Widget _totalPrice(BuildContext context) {
    return StreamBuilder(
        ...
        builder:
            (BuildContext context, AsyncSnapshot<DocumentSnapshot> snapshot) {
          if (snapshot.connectionState == ConnectionState.active) {
            final datas = snapshot.data['deluxe'];
            double totalPrice = 0;
  ...
  _addService() async {
    ...
    FirebaseFirestore.instance.collection('service').doc(uid).update({
      'deluxe': FieldValue.arrayUnion([serviceInfo]),
    }).whenComplete(() {
  ...
  _deleteService(Map<String, dynamic> serviceInfo) async {
    FirebaseFirestore.instance.collection('service').doc(uid).update({
      'deluxe': FieldValue.arrayRemove([serviceInfo])
    }).whenComplete(
