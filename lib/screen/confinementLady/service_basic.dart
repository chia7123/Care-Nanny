...
class BasicService extends StatelessWidget {
  BasicService({Key key}) : super(key: key);
                          ...
                          onDismissed: (direction) {
                            if (direction == DismissDirection.endToStart) {
                              Map<String, dynamic> serviceInfo = {
                                'title': data['basic'][index]['title'],
                                'price': data['basic'][index]['price'],
                              };
                              _deleteService(serviceInfo);
                            }
                          },
                          key: Key(data['basic'][index]['title']),
                          child: ListTile(
                            title: Text(
                                '${index + 1}. ${data['basic'][index]['title']}'),
                            subtitle: Text(
                                'RM ${double.parse(data['basic'][index]['price']).toStringAsFixed(2)}'),
                          ),
                        ),
                        Divider(
  ...
  Widget _totalPrice(BuildContext context) {
    return StreamBuilder(
        ...
        builder:
            (BuildContext context, AsyncSnapshot<DocumentSnapshot> snapshot) {
          if (snapshot.connectionState == ConnectionState.active) {
            final datas = snapshot.data['basic'];
            double totalPrice = 0;
  ...
  _addService() async {
    ...
    FirebaseFirestore.instance.collection('service').doc(uid).update({
      'basic': FieldValue.arrayUnion([serviceInfo]),
    }).whenComplete(() {
  ...
  _deleteService(Map<String, dynamic> serviceInfo) async {
    FirebaseFirestore.instance.collection('service').doc(uid).update({
      'basic': FieldValue.arrayRemove([serviceInfo])
    }).whenComplete(
